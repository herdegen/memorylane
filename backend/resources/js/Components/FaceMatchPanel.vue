<template>
  <div class="bg-white rounded-lg shadow-sm p-6">
    <div class="flex items-center justify-between mb-4">
      <h2 class="text-lg font-semibold text-gray-900">Identifier le visage</h2>
    ...
    <div class="mb-4">
      <label class="block text-sm font-medium text-gray-700 mb-1">Selectionner une personne</label>
      <div class="relative">
        <input
          v-model="searchQuery"
          type="text"
      ...
      <!-- People list -->
      <div v-if="filteredPeople.length > 0" class="mt-2 max-h-48 overflow-y-auto border border-gray-200 rounded-lg">
        <button
          v-for="person in filteredPeople"
          :key="person.id"
          @click="matchToPerson(person)"
          :disabled="matching"
          class="w-full px-3 py-2 text-left text-sm hover:bg-indigo-50 flex items-center gap-2 border-b border-gray-100 last:border-b-0"
        >
      ...
      <p v-else-if="searchQuery && !loadingPeople" class="mt-2 text-sm text-gray-500">
    ...
    <div class="flex items-center gap-2">
      <button
        ...
        class="flex-1 px-3 py-2 text-sm font-medium text-indigo-600 bg-indigo-50 rounded-lg hover:bg-indigo-100"
      >
        Nouvelle personne
      </button>
      <button
        @click="dismissFace"
        :disabled="matching"
        class="flex-1 px-3 py-2 text-sm font-medium text-gray-600 bg-gray-100 rounded-lg hover:bg-gray-200"
      >
        Ignorer
      </button>
    </div>
    ...
    <div v-if="showCreatePerson" class="mt-4 p-4 bg-gray-50 rounded-lg">
      <h3 class="text-sm font-medium text-gray-900 mb-3">Nouvelle personne</h3>
      <input
        v-model="newPersonName"
        type="text"
        placeholder="Nom"
        class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-indigo-500 focus:border-indigo-500 mb-3"
      ...
      <div class="flex gap-2">
        <button
          ...
          class="flex-1 px-3 py-2 text-sm font-medium text-white bg-indigo-600 rounded-lg hover:bg-indigo-700 disabled:opacity-50"
        >
          Creer et associer
        </button>
        <button
