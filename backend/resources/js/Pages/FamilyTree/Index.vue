<template>
  <AppLayout title="Arbre genealogique">
    <div class="h-[calc(100vh-4rem)] flex">
      <!-- Sidebar -->
      <div class="w-80 bg-white shadow-lg p-4 overflow-y-auto shrink-0">
      ...
      <div class="flex-1 relative bg-surface-50 overflow-hidden">
        ...
        <div v-if="treeNodes.length > 0" class="absolute bottom-4 right-4 flex flex-col gap-1">
          <button @click="zoom = Math.min(zoom * 1.2, 3)" class="w-8 h-8 bg-white rounded-sm shadow-sm flex items-center justify-center text-surface-600 hover:bg-surface-50">+</button>
          <button @click="zoom = Math.max(zoom / 1.2, 0.1)" class="w-8 h-8 bg-white rounded-sm shadow-sm flex items-center justify-center text-surface-600 hover:bg-surface-50">-</button>
          <button @click="resetView" class="w-8 h-8 bg-white rounded-sm shadow-sm flex items-center justify-center text-surface-600 hover:bg-surface-50 text-xs">R</button>
        </div>
      </div>
    </div>
  </AppLayout>
