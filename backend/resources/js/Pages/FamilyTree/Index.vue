      ...
      <div class="w-80 bg-white shadow-lg p-4 overflow-y-auto flex-shrink-0">
        ...
        <div class="mb-4">
          ...
          <div class="relative">
            ...
            >
              ...
              >
                {{ person.data.name }}
                <span v-if="person.data.birth_date" class="text-gray-400 ml-1">
        ...
        <div class="mb-4">
          <Link
            ...
            class="inline-flex items-center justify-center px-4 py-2 text-sm font-medium text-white bg-indigo-600 rounded-lg hover:bg-indigo-700 w-full"
          >
            Importer un GEDCOM
          </Link>
        </div>
        ...
        <div v-if="selectedPerson" class="mt-4 p-4 bg-gray-50 rounded-lg">
          <h3 class="font-semibold text-gray-900">{{ selectedPerson.data.name }}</h3>
          <p v-if="selectedPerson.data.birth_date" class="text-sm text-gray-500 mt-1">
          ...
          <p v-if="selectedPerson.data.death_date" class="text-sm text-gray-500">
      ...
      <div class="flex-1 relative bg-gray-50 overflow-hidden">
        ...
        <div v-if="!loading && treeNodes.length === 0" class="absolute inset-0 flex items-center justify-center">
          <div class="text-center">
            ...
            <p class="text-gray-500 mb-4">Aucune personne avec des liens familiaux.</p>
            <Link
              href="/family-tree/import"
        ...
        <div v-if="treeNodes.length > 0" class="absolute bottom-4 right-4 flex flex-col gap-1">
          <button @click="zoom = Math.min(zoom * 1.2, 3)" class="w-8 h-8 bg-white rounded shadow flex items-center justify-center text-gray-600 hover:bg-gray-50">+</button>
          <button @click="zoom = Math.max(zoom / 1.2, 0.1)" class="w-8 h-8 bg-white rounded shadow flex items-center justify-center text-gray-600 hover:bg-gray-50">-</button>
          <button @click="resetView" class="w-8 h-8 bg-white rounded shadow flex items-center justify-center text-gray-600 hover:bg-gray-50 text-xs">R</button>
        </div>
      </div>
    </div>
  </AppLayout>
